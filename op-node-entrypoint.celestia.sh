...
until [ "$(curl -s -w '%{http_code}' -o /dev/null "http://celestia-node:26659/header/1")" -eq 200 ]; do
  echo "waiting for light node to be ready"
  sleep 5
done


local_head=$(curl "http://celestia-node:26659/head" -s | jq -r '.header.height')
...
exec /opt/conduit/bin/op-node-entrypoint
